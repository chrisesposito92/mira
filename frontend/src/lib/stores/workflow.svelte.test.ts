...
// Dynamic import required for Svelte 5 runes module; typed via Awaited<typeof import(...)>
let store: Awaited<typeof import('./workflow.svelte.js')>['workflowStore'];
...
describe('WorkflowStore', () => {
	...
	it('handleWsMessage status sets thinking and adds message', () => {
		...
		const m = store.messages[0] as ChatStatusMessage;
		expect(m.type).toBe('status');
		expect(m.step).toBe('analyzing');
		expect(m.message).toBe('Analyzing use case...');
	});
	...
	it('handleWsMessage entities stops thinking and sets pendingEntities', () => {
		...
		expect(store.pendingDecisions).toEqual([]);
		expect(store.messages).toHaveLength(1);
		const m = store.messages[0] as ChatEntitiesMessage;
		expect(m.type).toBe('entities');
		expect(m.entity_type).toBe('product');
	});
	...
	it('handleWsMessage complete marks workflow completed', () => {
		(store as any).workflow = { id: 'wf-1', status: 'running' };
		const msg: WsServerMessage = {
			type: 'complete',
			message: 'All done!',
		};
	...
	it('handleWsMessage error marks workflow failed', () => {
		...
		expect(store.messages).toHaveLength(1);
		const m = store.messages[0] as ChatErrorMessage;
		expect(m.type).toBe('error');
		expect(m.message).toBe('Something went wrong');
	});
	...
	it('submitDecision accumulates decisions', () => {
		...
		store.submitDecision({ index: 0, action: 'approve' });
	...
	it('submitDecision deduplicates by index', () => {
		store.pendingEntities = {
			entity_type: 'product',
			entities: [{ name: 'A' }, { name: 'B' }],
		};
		...
		store.submitDecision({ index: 0, action: 'approve' });
		store.submitDecision({ index: 0, action: 'reject' });
		expect(store.pendingDecisions).toHaveLength(1);
		expect(store.pendingDecisions[0].action).toBe('approve');
	});

	// submitClarificationAnswers test
	it('submitClarificationAnswers sends and clears', () => {
		// Set up a mock ws via any cast (ws is private)
		(store as any).ws = { send: vi.fn(), disconnect: vi.fn() };
		...
		const answers = [{ free_text: 'Monthly' }];
		store.submitClarificationAnswers(answers);

		expect((store as any).ws.send).toHaveBeenCalledWith({
			type: 'clarify',
			answers,
		});
		expect(store.thinking).toBe(true);
		expect(store.pendingClarification).toBeNull();
		expect(store.messages).toHaveLength(1);
		const m = store.messages[0] as ChatUserClarificationMessage;
		expect(m.type).toBe('user_clarification');
		expect(m.answers).toEqual(answers);
	});
	...
	it('submitClarificationAnswers does nothing without ws', () => {
		...
		store.submitClarificationAnswers([{ selected_option: 0 }]);
	...
	it('clear resets all state', () => {
		...
		(store as any).workflow = { id: 'wf-1', status: 'running' };
		(store as any).workflows = [{ id: 'wf-1' }];
		store.thinking = true;
		...
		store.pendingClarification = [];
		store.pendingDecisions = [{ index: 0, action: 'approve' }];
	...
	it('loadMessages converts persisted messages to ChatMessages', async () => {
		...
		const m0 = store.messages[0] as ChatStatusMessage;
		expect(m0.type).toBe('status');
		expect(m0.step).toBe('analyzing');
